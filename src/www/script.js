...
function renderArticles(articles) {
    ...
    articles.forEach(article => {
        ...
        let icon = '📰';
        if (article.tags.includes('AI') || article.title.includes('AI')) icon = '🤖';
        else if (article.tags.includes('Finance') || article.title.includes('Market')) icon = '📊';
        else if (article.tags.includes('Crypto')) icon = '💰';
        else if (article.tags.includes('Tech')) icon = '💻';
        ...
        card.innerHTML = `
            ...
            <div class="card-content">
                <div class="match-reasons">
                    <span class="match-reason">Source: ${article.source}</span>
                    ${article.tags.slice(0, 2).map(tag => `<span class="match-reason">${tag}</span>`).join('')}
                </div>
                <div class="card-header">
                    <div>
                        ...
                        <div class="card-meta">
                            ...
                            <span><a href="${article.url}" target="_blank" style="color: inherit; text-decoration: none;">🔗 Read Original</a></span>
                        </div>
                    </div>
                </div>
                <p class="card-description">${article.summary || 'No summary available.'}</p>
                <div class="card-actions">
                    <button class="action-btn like">👍 Like</button>
